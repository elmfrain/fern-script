...
This was also a good opportunity to learn and apply good C practices by using concepts demonstrated from [Nic Barker](https://github.com/nicbarker) such as Memory Arenas, Bounds Checking, and leveraging macros effectively.

## Building

This project is built with one Makefile. Run:
...
* The root directoy contains the `Makefile` that is used to build the project
* Include `.h` files are in the `include` directory. There are no include directories defined in the `Makefile`, so each source file by reference them using the `include/some_header.h` pattern.
